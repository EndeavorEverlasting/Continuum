...
from .branch_topology import BranchTopologyError, parse_branch_policy
from .contracts import inspect_repository
from .execution_domains import ExecutionDomain, ExecutionDomainError, load_execution_domains
...
class TaskPacketError(RuntimeError):
    ...
    def to_dict(self) -> dict[str, Any]:
        return {"schema_version": 1, "command": "task", "status": "blocked", "blocker": {"code": self.code, "message": str(self)}}
...
class TaskScope:
    ...
    def to_dict(self) -> dict[str, list[str]]:
        return {"owned": list(self.owned), "forbidden": list(self.forbidden)}
...
class TaskPacket:
    ...
    def render_english(self) -> str:
        ...
        lines = [
            ...
            f"The task targets execution domain {self.execution_domain.name} over {self.execution_domain.transport} transport.",
        ...
        lines.extend(f"OWNED: {item}" for item in self.scope.owned)
        lines.extend(f"FORBIDDEN: {item}" for item in self.scope.forbidden)
        return "\n".join(lines)
...
def _normalize_scope(values: Iterable[str], *, label: str) -> tuple[str, ...]:
    ...
    if not normalized:
        raise TaskPacketError(f"scope.{label}_missing", f"At least one non-empty {label} scope entry is required.")
    if len(set(normalized)) != len(normalized):
        raise TaskPacketError(f"scope.{label}_duplicate", f"The {label} scope contains duplicate entries.")
    return normalized


def _derive_task_id(contract: RepositoryContract, git: GitEvidence, scope: TaskScope, execution_domain: ExecutionDomain) -> str:
    identity = {
    ...
    return f"task-{hashlib.sha256(payload.encode('utf-8')).hexdigest()[:16]}"


def compile_task_packet(root: Path, *, owned_scope: Iterable[str], forbidden_scope: Iterable[str], domain_name: str | None = None, task_id: str | None = None, recent_limit: int = 5) -> TaskPacket:
    owned = _normalize_scope(owned_scope, label="owned")
    forbidden = _normalize_scope(forbidden_scope, label="forbidden")
    report = inspect_repository(root)
    if not report.ok:
        detail = "; ".join(f"{check.check_id}: {check.message}" for check in report.checks if not check.passed)
        raise TaskPacketError("contract.invalid", detail or "The repository contract is invalid.")
    ...
    try:
        branch_policy = parse_branch_policy(document).to_dict()
    ...
    if git.repository_root != contract.root:
        raise TaskPacketError("git.root_mismatch", f"The contract root {contract.root} does not match the Git root {git.repository_root}.")
    scope = TaskScope(owned=owned, forbidden=forbidden)
    resolved_task_id = task_id.strip() if task_id else _derive_task_id(contract, git, scope, execution_domain)
    if not resolved_task_id:
        raise TaskPacketError("task_id.empty", "The task ID must not be empty.")
    return TaskPacket(resolved_task_id, contract, git, scope, execution_domain)
